...
public class PageReader {
	 ...
	 * boundaries. uses a temporary image to store intermediate crop
	 *
	...
	public static ParsedReport scanImage(GradeReport selectedImage, double nameSelectedLeft, double nameSelectedRight,
		...
		Set<Course> foundCourses = null;// = new HashSet<>();
			...
			try {
				if (headerFound) {
					...
					if (!result.trim().equals("")) {
						...
						logger.debug("grade found at {}", gradeCol.toString());
						...
						logger.debug("detected course {}", course);
						...
						try {
							...
							logger.error("Problem adding course", e);
						}
					}
				}

				if (!gpaFound) {
					logger.trace("searching {} for gpa", f);
					String wholeLine = instance.doOCR(f);
				...
				if (!headerFound) {
					...
					logger.trace("read whole line: {}", wholeLine.trim());
					double score = FuzzySearch.ratio(
							"Year Marking Period Course Course Number Percent Grade Grade Scale Cred. Attempted Cred. Earned GPA PTS Weighted GPA Affects GPA Teacher",
							wholeLine.trim());
	...
	private static boolean isGPALine(String wholeLine) {
		// logger.trace("GPA search score: {}", FuzzySearch.partialRatio(wholeLine,
		// "Cumulative GPA: "));
	...
	public static List<BoxResult> getBoxes(File f) throws IOException, TesseractException {
		...
		for (int i = 0; i < result.size(); i++) {
			...
			b.word = words[i];
			boxes.add(b);
			logger.info(String.format("Box[%d]: x=%d, y=%d, w=%d, h=%d word=%s", i, rect.x, rect.y, rect.width, rect.height,
					words[i]));
			// Imgproc.line(img, new org.opencv.core.Point(0, xcoords.get(i) * img.rows()),
			// new Point(img.cols(),
	...
	static Column findColumn(String name, List<BoxResult> boxes) {
		for (int i = 0; i < boxes.size(); i++) {
			...
			if (FuzzySearch.ratio(r.word, name) > 80) {
				Column c = new Column();
				c.begin = r.x;
				if (i == boxes.size() - 1) {
					c.end = c.begin + r.w;
	...
	static public class Column {
		...
		public String toString() {
			return "Column{" + "begin=" + begin + ", end=" + end + ", header='" + header + '\'' + '}';
		}
